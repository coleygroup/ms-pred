...
def get_args():
    ...
    parser.add_argument(
        ...
        default="results/frag_only_nist20/split_1_rnd1/version_0/best.ckpt",
    )
...
def predict():
    ...
    gpu = kwargs["gpu"]
    model = frag_model.FragOnlyModel.load_from_checkpoint(best_checkpoint, map_location='cpu')
    avail_gpu_num = torch.cuda.device_count()

    logging.info(f"Loaded model with from {best_checkpoint}")
    if kwargs["num_decoys"] == 0:
        save_path = save_dir / "frag_preds.hdf5"
    else:
        save_dir = save_dir / "decoy_frag_preds"
        save_path = save_dir / f"decoy_frag_preds.hdf5"
    ...
    with torch.no_grad():
        ...
        def prepare_entry(entry):
            ...
            if num_decoys > 0:  # decoys only
                ...
                for i, decoy_smi in enumerate(decoy_smis):
                    for colli_eng in collision_energies:
                        ...
                        tup_to_process.append((
                            decoy_smi, f'pred_{name}', colli_eng_val, f'decoy {i}', adduct,
                        ))
                ...
                for colli_eng in collision_energies:
                    colli_eng_val = common.collision_energy_to_float(colli_eng)  # str to float
                    tup_to_process.append((
                        smi, f'pred_{name}', colli_eng_val, None, adduct,
                    ))
        ...
        def producer_func(batch):
            ...
            smi, name, colli_eng_val, decoy_label, adduct = list(zip(*batch))
            pred = model.predict(
                smi,
            ...
            patterns, patterns_count = pred["fragment_patterns"], pred["patterns_count"]
            return_list = []
            for i, (_smi, _name, _colli_eng_val, _decoy_label, _adduct) in \
                    enumerate(zip(smi, name, colli_eng_val, decoy_label, adduct)):
                count = patterns_count[i].item()
                patterns = pred["fragment_patterns"][i, :count].cpu().numpy()
        ...
        def write_func(out_entries):
            specdb = common.PredSpecDB(
                save_path, mode='w', num_h5s=kwargs["num_decoys"] if kwargs["num_decoys"] > 1 else 1, has_frags=True,)
