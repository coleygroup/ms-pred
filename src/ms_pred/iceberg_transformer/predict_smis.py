...
from ms_pred.iceberg_transformer import inten_model, frag_model, joint_model
...
def predict():
    ...
    frag_checkpoint = kwargs["frag_checkpoint"]
    inten_checkpoint = kwargs["inten_checkpoint"]

    gpu = kwargs["gpu"]
    inten_model_obj = inten_model.IntenModel.load_from_checkpoint(inten_checkpoint) #, map_location="cuda" if gpu else "cpu")
    frag_model_obj = frag_model.FragOnlyModel.load_from_checkpoint(frag_checkpoint) #, map_location="cuda" if gpu else "cpu")
    ...
    logging.info(
        f"Loaded frag / inten models from {frag_checkpoint} & {inten_checkpoint}"
    )

    model = joint_model.JointModel(
        frag_model_obj=frag_model_obj, inten_model_obj=inten_model_obj
    )
