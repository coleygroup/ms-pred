...
from ms_pred.iceberg_transformer import dataset, inten_model
...
def add_frag_train_args(parser):
    ...
    parser.add_argument("--max-broken-bonds", default=6, action="store", type=int)
    parser.add_argument("--num-edge-dis", default=10, action="store", type=int)
    parser.add_argument("--multi-hop-max-dist", default=3, action="store", type=int)
    parser.add_argument("--max-frags", default=100, action="store", type=int)
...
def train_model():
    ...
    magma_folder = Path(kwargs["magma_folder"])
    magma_tree_h5 = common.PredSpecDB(magma_folder)
    name_to_keys = {}
    ...
    train_dataset = dataset.IntenContrDataset(
        train_df,
        magma_h5=magma_folder,
    ...
    val_dataset = dataset.IntenContrDataset(
        val_df,
        magma_h5=magma_folder,
        magma_map=name_to_keys,
        root_encode=root_encode,
    ...
    test_dataset = dataset.IntenContrDataset(
        test_df,
        magma_h5=magma_folder,
        magma_map=name_to_keys,
        root_encode=root_encode,
    ...
    if kwargs["train_checkpoint"]:
        ...
        model = inten_model.IntenModel.load_from_checkpoint(train_checkpoint)
        logging.info(
    ...
    model = inten_model.IntenModel.load_from_checkpoint(test_checkpoint)
    # Force contrastive learning params
    model.sk_tau = kwargs["sk_tau"]
    model.contr_weight = kwargs["contr_weight"]
